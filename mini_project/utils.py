...
def step_envs(*, cpu_actions, envs, episode_rewards, episode_costs, result_recorder, total_steps, total_episodes,
              device):
    """Step the vectorized environments for one step. Process the reward
    recording and terminal states."""
    obs, reward, done, info = envs.step(cpu_actions)
    episode_costs += np.array([v['cost'] for v in info]).reshape(-1, 1)
    ...
    for idx, d in enumerate(done):
        ...
        if d:  # the episode is done
            ...
            result_recorder["episode_cost"].append(episode_costs[idx].copy())
            if "route_completion" in info[idx]:
                result_recorder["route_completion"].append(info[idx]["route_completion"])
            if "arrive_dest" in info[idx]:
                result_recorder["success_rate"].append(info[idx]["arrive_dest"])
            if "out_of_road" in info[idx]:
                result_recorder["out_of_road_rate"].append(info[idx]["out_of_road"])
    ...
    episode_costs *= masks.reshape(-1, 1)
    ...
    return obs, reward, done, info, masks, total_episodes, total_steps, episode_rewards, episode_costs
